module fifo_rdinput_agent extends uvm_agent;
...
function void build_phase(uvm_phase phase);
  ...
  if(!(uvm_config_db#(fifo_config)::get(this,"","fifo_config", rdinp_cfg))) begin
    `uvm_fatal(get_type_name(),"RD_INPUT_AGENT_CONFIG_NOT_CONNECTED") 
  if(rdinp.wrinput_agent_is_active == UVM_ACTIVE) begin
...
function void connetc_phase(uvm_phase phase);
  if(rdinp.wrinput_agent_is_active == UVM_ACTIVE) begin
    rddrv.seq_item_port.connect(rdinmon.seq_item_export);
  end
endfunction
